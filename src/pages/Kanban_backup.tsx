...
export default function KanbanPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Selecionar primeiro funil se necessÃ¡rio
  ...
  // Atualiza apenas os leads sem recarregar a pÃ¡gina
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!newEtapaId) {
      toast.error("Etapa de destino invÃ¡lida");
      return;
    }
    ...
    // Verificar se jÃ¡ estÃ¡ na mesma etapa para evitar operaÃ§Ãµes desnecessÃ¡rias
    ...
    setLeads(leads =>
      leads.map(l => l.id === leadId ? { ...l, etapa_id: newEtapaId, funil_id: etapaDestino.funil_id } : l)
    );

    try {
      // Atualizar no banco sem bloquear a UI
      ...
      // Reverter a mudanÃ§a local em caso de erro
      setLeads(leads =>
        leads.map(l => l.id === leadId ? { ...l, etapa_id: originalEtapaId, funil_id: originalFunilId } : l)
      );
    }
  };
