...
import { Users, Target, Calendar, DollarSign, TrendingUp, Percent, MessageCircle } from "lucide-react";
...
export function ProspeccaoKPIs({ data, channelType, isLoading }: Props) {
  const totals = data.reduce(
    ...
    { leads: 0, responses: 0, opportunities: 0, meetings: 0, sales: 0, gross: 0, adSpend: 0 }
  );

  const pctSafe = (a: number, b: number) => b > 0 ? ((a / b) * 100).toFixed(1) : "0";
  ...
  const cards = [
    ...
    { label: "% Lead → Resposta", value: `${pctSafe(totals.responses, totals.leads)}%`, icon: Percent, color: "text-amber-500" },
    { label: "Respostas", value: totals.responses.toString(), icon: MessageCircle, color: "text-cyan-500" },
    { label: "% Resposta → Oport.", value: `${pctSafe(totals.opportunities, totals.responses)}%`, icon: Percent, color: "text-amber-500" },
    { label: "Oportunidades", value: totals.opportunities.toString(), icon: Target, color: "text-emerald-500" },
    { label: "% Oport. → Reunião", value: `${pctSafe(totals.meetings, totals.opportunities)}%`, icon: Percent, color: "text-amber-500" },
    { label: "Reuniões", value: totals.meetings.toString(), icon: Calendar, color: "text-purple-500" },
    { label: "% Reunião → Venda", value: `${pctSafe(totals.sales, totals.meetings)}%`, icon: Percent, color: "text-amber-500" },
